...
const App = () => {
  ...
  const handleDelete = (id) => {
    const delet = products.filter((elem) => elem.id !== id)
    return setProducts(delet)
  }

  return (
    <div>
      <h1 className="pt-10 m-4 text-3xl font-bold text-white">
        Product Card App
      </h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {products.map((product) => (
          <ProductCard key={product.id} {...product} del={handleDelete} />
        ))}
      </div>
    </div>
  );
